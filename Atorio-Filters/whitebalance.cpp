...
static vec3 srgb2lrgb(vec3 c) {
	c.x = (c.x <= 0.04045) ? c.x * 0.0773993808 : pow((c.x + 0.055) * 0.94786729857, 2.4);
	c.y = (c.y <= 0.04045) ? c.y * 0.0773993808 : pow((c.y + 0.055) * 0.94786729857, 2.4);
	c.z = (c.z <= 0.04045) ? c.z * 0.0773993808 : pow((c.z + 0.055) * 0.94786729857, 2.4);
	return c;
}
...
static vec4 temp(float K) {
    vec4 color = { 0, 0, 0, 1 };
    if (K < 1000.0F) {
        return { 1.0F, .03133630F, 0.0F };
    }
    if (K > 40000.0F) {
        return { 0.3567632F, 0.4747332F, 1.0F };
    }
    if (K > 6700.0F) {
        color.x = (float) (36331.1572487 * (powf(K - 2697.65116524, -1.31345819667)) + 0.32083009448);
        color.y = (float) (21477.5773003 * (powf(K - 1919.02861119, -1.25528592041)) + 0.436548753055);
        color.z = 1;
    }
    else {
        color.x = 1.0F;
        color.y = (float) (-4.6191186647e-9 * (powf(K - 999.999997571, 2.14602263521)) - 0.223195250169);
        color.z = (K <= 1900) ? 0.0F : (float) (445664994.867 * (powf(K - 23.8434596537, -2.85847682864)) + 0.000249043821817 * K - 0.657104011847);
    }
    return vec4{ 1,1,1,1 };
}
